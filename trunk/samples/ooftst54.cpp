...
// -------------------------------------------------------
//             d b L o o k u p R e f
// -------------------------------------------------------

/**
	Link to record, including adding if necessary.
	\param inLongDescToAdd is optional string
*/
void 
dbLookupRef::addOrFind(const char* inLookupDesc)
{
	dbLookup* pointsTo = *this;
	if (!setRelatedRecordMatching(pointsTo->ShortDesc==inLookupDesc) ) {
		pointsTo->newRecord();
		pointsTo->ShortDesc = inLookupDesc;
	}
}
// -------------------------------------------------------
//             d b P a r t y
...
// 		Roles->addRole("Webmaster2", "Acme Widgets Inc.");  // crashes 
...
int main()
{
	cout << "OOFILE Validation Suite - Test 54\n"
	...
	try {
		...
		dbPerson		Person;
		dbOrg			Org;
		dbLookup		RoleDescription;
		
	
		RoleDescription.setName("Role Description");   // generic table needs renaming
	...
	// relationship between two tables	
		...
		dbRelationship( Role.Desc, RoleDescription.LookupUser );
	...
	// A single Person may relate to many Orgs and vice versa	
		...
		#ifdef TESTING_DBASE
			#ifdef _Macintosh
				const char* kExistsName =  ":ooftst54:Person.dbf";
				const char* kDatabaseName = ":ooftst54:";
			#else
		...
		#else
			const char* kDatabaseName = "ooftst54.db";
